...
FILE_OUTPUT=/app/stats/stats_logs/stats.txt
...
while true 
  do
		...
		echo -e "\t" >> $FILE_OUTPUT
		stat1=$(MYSQL_PWD=password mysql -uroot -hvsDB -P3306 < ./stats/stats1.sql)
		echo -e "$stat1" >> $FILE_OUTPUT

		echo "--------------------------------------------------------------------------------------------------------------------------------------" >> $FILE_OUTPUT
		echo -e "\n\t(2) Statistic about: Query Latency - $(timestamp)" >> $FILE_OUTPUT
		echo -e "\t" >> $FILE_OUTPUT
		stat2=$(MYSQL_PWD=password mysql -uroot -hvsDB -P3306 < ./stats/stats2.sql)
		echo "$stat2" >> $FILE_OUTPUT

		echo "--------------------------------------------------------------------------------------------------------------------------------------" >> $FILE_OUTPUT
		echo -e "\n\t(3) Statistic about: Errors Occurred - $(timestamp)" >> $FILE_OUTPUT 
		echo -e "\t" >> $FILE_OUTPUT
		stat3=$(MYSQL_PWD=password mysql -uroot -hvsDB -P3306 < ./stats/stats3.sql)
		echo "$stat3" >> $FILE_OUTPUT

		echo "--------------------------------------------------------------------------------------------------------------------------------------" >> $FILE_OUTPUT
		echo -e "\n\t(4) Statistic about: Query per second - $(timestamp)" >> $FILE_OUTPUT  
		echo -e "\t" >> $FILE_OUTPUT	
		stat4=$(MYSQL_PWD=password mysql -uroot -hvsDB -P3306 < ./stats/stats4.sql) 
		...
		MYSQL_PWD=password mysql -N -uroot -hvsDB -P3306 < ./stats/stats6_7.sql | awk '{ORS=" "; byte=$1 /1024/1024; print byte " MB \t"}' >> $FILE_OUTPUT
